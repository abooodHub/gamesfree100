...
function gameCard(game, isNew = false) {
    ...
    // Ubisoft: game[4] (إذا كان موجوداً)
    ...
    if (endDate) {
        try {
            ...
            if (timeLeft > 0) {
                ...
                let timeText = '';
                if (days > 0) {
                    timeText = `${days} ${storeNames[lang].daysLeft}`;
                } else if (hours > 0) {
                    timeText = `${hours} ${storeNames[lang].hoursLeft}`;
                } else {
                    timeText = storeNames[lang].endingSoon;
                }
...
function updateNavigationLinks() {
    // تحديث روابط Navigation في Header
        ...
        if (href) {
            switch (href) {
                case 'index.html':
                    ...
                    break;
                case 'reviews.html':
                    link.textContent = lang === 'ar' ? 'المراجعات' : 'Reviews';
                    break;
                case 'guides.html':
                    link.textContent = lang === 'ar' ? 'الدليل' : 'Guides';
                    break;
                case 'about.html':
                    link.textContent = lang === 'ar' ? 'عن الموقع' : 'About';
                    break;
            }
        }
    });
...
function acceptCookies() {
    ...
    showGameNotification('تم قبول ملفات تعريف الارتباط', 'success');
}

function rejectCookies() {
    localStorage.setItem('cookieConsent', 'rejected');
    document.getElementById('cookieConsent').style.display = 'none';
    showGameNotification('تم رفض ملفات تعريف الارتباط', 'info');
}
