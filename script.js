        ...
        fetch(file + cacheBuster)
            ...
            .then(data => {
                ...
                if (loaded === totalFiles) {
                    ...
                    // تفعيل الفحص الدوري للألعاب المنتهية (مرة واحدة فقط)
...
// فحص دوري للألعاب المنتهية وإزالتها من العرض
function checkAndRemoveExpiredGames() {
    const currentCount = allGames.length;
    const previousCount = currentCount;

    // إعادة دمج الألعاب مع تصفية المنتهية
    mergeAllGames();

    const newCount = allGames.length;

    // إذا تغير العدد، يعني أن هناك ألعاب منتهية تم إزالتها
    if (newCount < previousCount) {
        const removedCount = previousCount - newCount;
        console.log(`🗑️ تم إزالة ${removedCount} لعبة منتهية من العرض`);
...
// إعداد الفحص الدوري للألعاب المنتهية
function setupExpiredGamesCheck() {
    // فحص كل دقيقة (60000 ms) للحذف الفوري
    const checkInterval = 60 * 1000;

    setInterval(() => {
        console.log('🔍 فحص الألعاب المنتهية...');
        checkAndRemoveExpiredGames();
    }, checkInterval);

    console.log('✅ تم تفعيل الفحص الدوري للألعاب المنتهية كل دقيقة');
}


// دالة مساعدة للتحقق من انتهاء صلاحية الخصم
function isGameExpired(game, storeKey) {
    let endDate = null;
    ...
    if (!endDate) {
        // fallback: إذا كانت Game من Steam وخصمها 100% لكن end_date مفقود
        // (Steam أحياناً يرجع null في البيانات، لذلك نستخدم مدة افتراضية قصيرة لتجنب بقاء الألعاب منتهية لفترة طويلة)
        if (storeKey === 'steam') {
            const discount = game[6];
            if (typeof discount === 'string' && discount.includes('100%')) {
                const updateStr = gamesData && gamesData.steam ? gamesData.steam.update_time : null;
                if (updateStr && typeof updateStr === 'string') {
                    const updateEndStr = updateStr.trim();
                    let iso = updateEndStr;
                    if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(updateEndStr)) {
                        iso = updateEndStr.replace(' ', 'T') + 'Z';
                    }
                    const updateDateObj = new Date(iso);
                    if (!isNaN(updateDateObj.getTime())) {
                        const maxAgeMs = 12 * 60 * 60 * 1000; // 12 ساعة
                        return updateDateObj.getTime() + maxAgeMs <= Date.now();
                    }
                }
            }
        }
    ...
    try {
        ...
        // Steam غالباً يخزن: "YYYY-MM-DD HH:MM:SS"
        if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(endStr)) {
            iso = endStr.replace(' ', 'T') + 'Z';
        } else if (/^\d{4}-\d{2}-\d{2}$/.test(endStr)) {
            iso = endStr + 'T00:00:00Z';
        } else if (/^\d{4}-\d{2}-\d{2}T/.test(endStr) && !endStr.endsWith('Z')) {
            // إذا كانت الصيغة ISO بدون timezone
            iso = endStr + 'Z';
        }

        const endDateObj = new Date(iso);
        if (isNaN(endDateObj.getTime())) {
            // محاولة بديلة بدون افتراض UTC
            const alt = endStr.replace(' ', 'T');
            const altDate = new Date(alt);
            if (isNaN(altDate.getTime())) return false;
            return altDate <= new Date();
        }

        return endDateObj <= new Date(); // منتهية إذا كان التاريخ في الماضي أو الحاضر
    } catch (e) {
        console.log('خطأ في تحليل تاريخ الانتهاء:', e);
        return false; // في حالة الخطأ، لا نعتبر اللعبة منتهية
    }
}

function mergeAllGames() {
    allGames = [];
    const seen = new Set(); // منع التكرار بالاسم+الرابط
    Object.keys(gamesData).forEach(key => {
        const d = gamesData[key];
        if (!d) return;
        const add = (g) => {
            const uniqueKey = `${g[0]}||${g[1]}`;
            if (seen.has(uniqueKey)) return; // تجاهل المكرر
            // فحص إذا كانت اللعبة منتهية قبل إضافتها
            if (!isGameExpired(g, key)) {
    ...
    console.log(`تم دمج ${allGames.length} لعبة (خصم 100% فقط - بعد استبعاد المنتهية والمكررة)`);
}
