...
// Theme Management - إدارة المظهر
function initThemeToggle() {
    const themeToggle = $('#themeToggle');
    if (!themeToggle) return;

    // Load saved theme
    const savedTheme = localStorage.getItem('games100_theme') || 'dark';
    ...
    themeToggle.addEventListener('click', () => {
        ...
        document.documentElement.setAttribute('data-theme', currentTheme);
        localStorage.setItem('games100_theme', currentTheme);
...
// Game Filter Buttons - أزرار تصفية الألعاب
...
// Update Store Counts - تحديث عداد المتاجر
function updateStoreCounts() {
    // Reset counts
    Object.keys(storeCounts).forEach(key => storeCounts[key] = 0);
    
    // Count games per store
    allGames.forEach(game => {
        const store = game._store;
        if (store && storeCounts.hasOwnProperty(store)) {
            storeCounts[store]++;
        }
        storeCounts.all++;
    });
    ...
    $$('.store-filter-tab').forEach(tab => {
        ...
        if (countEl && storeCounts.hasOwnProperty(store)) {
            countEl.textContent = storeCounts[store];
        }
    });
}
...
function renderGames() {
    ...
    if (currentFilter === 'free') {
        filteredGames = filteredGames.filter(game => {
            const discountInfo = game[6] || game[4] || '';
            return discountInfo.includes('100%') || discountInfo.includes('مجاني') || discountInfo.toLowerCase().includes('free');
        });
    } else if (currentFilter === 'discount') {
        filteredGames = filteredGames.filter(game => {
            const discountInfo = game[6] || game[4] || '';
            return discountInfo.includes('%') && !discountInfo.includes('100%');
        });
    }
