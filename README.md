...
├── deals.json                  # ملف العرض العام الذي تقرؤه الواجهة
...
الواجهة لا تحمّل ملفات المصادر؛ بل تقرأ `deals.json` فقط. يعرض البناء ألعاب Steam المجانية دائماً بشارة مستقلة، والعروض الفعالة ذات الخصم 100%، وتعرض الواجهة 48 لعبة في كل دفعة، ويتحقق من روابط HTTPS ونطاق المتجر وبنية البيانات قبل النشر.
...
تُنشر حالة كل متجر بعد كل محاولة تحديث حتى لو لم تتغير الألعاب. تعرض الواجهة آخر محاولة وآخر تحقق ناجح، وتميز فشل الجلب عن تأخر التحديث لأكثر من 12 ساعة. تُحدّث الحالة في المتصفح كل 5 دقائق. فشل جلب متجر لا يمنع تحديث المتجر الآخر، وتبقى آخر بيانات متاحة مع التنبيه. كما تُحذف سجلات Actions المكتملة الأقدم من 14 يومًا تلقائيًا مع بقاء سير العمل فعالًا.
