...
def parse_end_date_text(text: str) -> Optional[str]:
    try:
        ...
        if m:
            ...
            if not y_str and dt < now - datetime.timedelta(days=180):
...
def fetch_Steam_json_response(url: str) -> dict:
    ...
    for _ in range(3):
        try:
            ...
            payload = resp.json()
            if not isinstance(payload, dict) or payload.get("success") != 1 or not isinstance(payload.get("results_html"), str):
...
def get_free_goods(start: int, append_list: bool = False,
    ...
    while retry_time >= 0:
        try:
            ...
            response_json = fetch_Steam_json_response(url.format(pos=start) + "&cc=us&l=english&category1=998")
            ...
            for game in all_games:
                try:
                    ...
                    if not is_discounted:
                        price_elem = game.select_one(".discount_final_price, .search_price")
                        if price_elem:
                            price_text = price_elem.get_text().strip().lower()
                            if price_text in {"free", "free to play", "0", "مجاني"}:
                                is_free = True
                                new_price = "مجاني"
                                old_price = ""
...
def is_game_expired(game):
    ...
    try:
        ...
        is_expired = end_datetime <= datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)
        if is_expired:
    ...
    try:
        ...
        for attempt in range(3):
            try:
                ...
                still_active = (discount_percent == 100 and final_price == 0)
                if not still_active:
...
def main():
    ...
    # البحث الموجّه عن المجاني مؤقتاً بدلاً من آلاف العروض المدفوعة.
    print("🔍 البحث عن عروض Steam المجانية المؤقتة...")
    get_free_goods(0, True, use_free_temporary=True)
    ...
    for game in existing_discounted_games:
        if len(game) < 7 or str(game[6]).strip() not in {"100%", "-100%"}:
            continue
        if len(game) > 1:
            appid = extract_appid_from_url(game[1])
            if appid and appid not in all_new_appids:
